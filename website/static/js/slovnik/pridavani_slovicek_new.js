...
let novy_jazyk_button = document.getElementById("new_jazyk_dropdown")
let potvrdit_jazyky_btn = document.getElementById("potvrdit_jazyky")

novy_jazyk_button.addEventListener("click", novy_jazyk_dropdown)
potvrdit_jazyky_btn.addEventListener("click", potvrdit_jazyky)
...
function potvrdit_jazyky() {
    // zkontrolovat jestli jsou vybrany smysluplne
    novy_jazyk_button.disabled = true
    potvrdit_jazyky_btn.disabled = true
    let dropdowny = document.getElementsByName("dropdown_jazyka")
    for (let i =0; i<dropdowny.length;i++) {
        dropdowny[i].disabled = true
    }
