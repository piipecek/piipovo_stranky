...
class UceniManager:
    ...
    def get_next_data(self):
        ...
        if all([zaznam["write"] for zaznam in self.data_o_zkouseni]):
            return None
        else:
    ...
    def check_choose(self, datum_puvodniho, datum_vybraneho):
        for i, zaznam in enumerate(self.data_o_zkouseni):
            if zaznam["datum"] == datum_puvodniho:
                if datum_puvodniho == datum_vybraneho:
                    ...
                    self.data_o_zkouseni[i]["showcase"] = False
        ...
        if datum_puvodniho == datum_vybraneho:
            ...
            for i, zaznam in enumerate(self.data_o_zkouseni):
                if zaznam["datum"] == datum_puvodniho:
                    self.data_o_zkouseni[i]["showcase"] = False
            message = "spatne, spravne bylo " + Slovicko.get_by_timestamp(datum_puvodniho).pretty(self.jazyk)
            category = "error"
        self.zapsat_do_souboru()
        return message, category
        """
    ...
    def check_write(self, datum, string):
        ...
        for i, zaznam in enumerate(self.data_o_zkouseni):
            if zaznam["datum"]  == datum:
                if result:
                    ...
                    self.data_o_zkouseni[i]["choose"] = False
        ...
        if result:
            for i, zaznam in enumerate(self.data_o_zkouseni):
                if zaznam["datum"] == datum:
                    self.data_o_zkouseni[i]["write"] = True
            message, category = "Správně!", "correct"
        else:
            for i, zaznam in enumerate(self.data_o_zkouseni):
                if zaznam["datum"] == datum:
                    self.data_o_zkouseni[i]["choose"] = False
            message, category =  "spatne, spravne bylo " + Slovicko.get_by_timestamp(datum).pretty(self.jazyk), "error"
        self.zapsat_do_souboru()
        return message, category
"""
